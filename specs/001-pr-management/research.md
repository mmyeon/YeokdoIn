...
## R1. 0.5kg 단위 검증을 `double precision` 컬럼에서 신뢰할 수 있는가

**Decision**: `weight * 2 = trunc(weight * 2)` 를 CHECK 제약으로 쓴다. 컬럼 타입은 바꾸지 않는다.

**Rationale**: 0.5는 이진 부동소수점에서 **정확히 표현된다**(2^-1). 따라서 0.5의 배수도 오차 없이
표현되고, 2를 곱한 값은 정확한 정수가 된다. 로컬 Postgres에서 확인한 결과:
...
| 52.4 | ❌ (의도대로 거부) |
| 62.25 | ❌ (의도대로 거부) |
| 0.1 + 0.2 (= 0.30000000000000004) | ❌ (의도대로 거부) |

**Alternatives considered**:

- `numeric(5,1)`로 컬럼 타입 변경 — 의미상 더 정확하지만 `ALTER COLUMN TYPE`은 테이블 재작성을
  유발하고 `types_db.ts` 재생성과 기존 코드의 `number` 가정을 흔든다. 얻는 것에 비해 위험이 크다.
- `weight % 0.5 = 0` — 부동소수점 나머지 연산은 타입에 따라 오차가 나므로 `trunc` 비교보다 불안정하다.
...
**Rationale**: 로컬 DB 실측 결과 두 테이블 모두 위반 0건이다.

| 테이블 | 총 행 | weight ≤ 0 | 0.5 배수 아님 | 1000 초과 | 미래 날짜 |
|---|---|---|---|---|---|
...
| FR-010 (0.5kg 단위) | ❌ 없음 | model + 서버 액션 + CHECK (R1) |
