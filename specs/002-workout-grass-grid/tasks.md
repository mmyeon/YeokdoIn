...
- [ ] T022 [US3] `WorkoutGrid.tsx` 에서 `usePrograms()` 의 `isLoading` 분기를 추가한다. 홈의 기존 `HomeSkeleton` 톤과 맞춘다
- [ ] T023 [US3] 기록 0건일 때 빈 그리드와 함께 **칸을 켜는 방법**(프로그램 입력하러 가기)을 안내한다 (FR-009). 오류처럼 보이면 안 된다
- [ ] T024 [US3] `isError` 일 때 오류 안내와 `refetch()` 재시도 버튼을 렌더한다 (FR-009). **빈 그리드로 위장하지 않는다** — 이 분기가 빈 상태와 같은 화면을 그리면 안 된다
...
- [ ] T032 실기기(모바일 브라우저)에서 칸 터치를 실제로 해본다. plan.md Complexity Tracking 이 기록한 ~9.7px 타깃이 실사용에서 견딜 만한지 판단하고, **못 견디면 26주 결정을 다시 여는 이슈를 만든다** — 그 자리에서 열 수를 바꾸지 않는다(명세 변경이 먼저다)
